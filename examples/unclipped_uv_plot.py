...
def draw(pixel_width, pixel_height, frame_no, frame_count):
    ...
    axes.draw()
    plot = plots.Plot3dXYZofUV(axes).of_function(lambda u, v :u*math.cos(v), lambda u, v :u*math.sin(v), lambda u, v :v, u_extent=(0, 1), v_extent=(-math.pi, 3*math.pi), precision=100).fill(colormap.ViridianMap(0, 1))#.stroke([0.2, 0, 0, 1], u_divs=4, v_divs=30)
    plot.plot()

    return camera_object

make_image.make_blender_image("unclipped_uv_plot", draw, 500, 500)